...
def align(source_sents, target_sents, mean, variance, penalty, option='char'):
    ...
    _align(list(source_sents_lens), list(target_sents_lens), mean, variance, penalty):
...
def file_len(filename, option='char'):
    ...
    return sum(text_len(i, option=option) for i in filename)
...
def calculate_gacha_variance(srcfile, trgfile, option='char'):
    ...
    trg = trgfile
    print(src)
    srclen = text_len(src, option=option)
    trglen = text_len(trg, option=option)
...
def main(source_corpus, target_corpus, mean=parameters.mean, 
    ...
    trg = target_corpus
    pairings = []
    for sentence_pair in align(src,trg, mean, variance, penalty):
        pairings.append(sentence_pair)
    return pairings
